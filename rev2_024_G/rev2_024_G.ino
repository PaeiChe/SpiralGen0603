/*
   VORTEX GENERATOR
   2019 Spherical Sound Society
   2020 May Goes Open Source
   Keeping the hardware closed source
   To Get PCBs, kits or build units, please visit
   https://www.tindie.com/stores/jc2046/

   CONNECTIONS (DAC MCP4822)
...
//We are using the LogTable to transform the linear response of the pots
// into logaritmic one
...
//analogic pins
#define A_AttackPin           A2
#define A_DecayPin            A3
#define B_AttackPin           A0
#define B_DecayPin            A1
...
//ADSR STATES 0=off, 1=attack phase, 2=decay phase
void setup()
{
  ...
  // We init the module with both cahennels looping
  bool isBInterrupt = true;
  bool isAInterrupt = true;
}
...
void loop()
{
  ...
  readPotsCycle++;
  if (readPotsCycle == readPotsEveryXCycles) {
    readPotsCycle = 0;
    readAPots();
    readBPots();
  }
  ...
  if (!easterMode)runglerBchannel();

  //TODO 1024 hardcoded here. it should be calculated against lenghtfactor
  ...
  MCPDAC.setVoltage(CHANNEL_A, A_OutputVoltage >> lengthFactor);
...
void envelopeManager() {
  ...
  // 10 miliseconds long triggers on end of cycle
...
void readAPots() {
  ...
  // TODO IMPLEMENT ANALOGREADFAST
  // Add POT A+Attack CV, then divide by 2 (>>1) and then check what logtable value is
...
void readBPots() {
  ...
  int BCV_Read = analogReadFast(ATT_B_CV_Pin);
  /* not ready yet, but you can activate it manually at the init of the variables
   *  
  if (BCV_Read > 950) {
    easterMode = true;
    BCV_Read = 0;
  ...
  */
   ATT_B_CV = softDebounce(BCV_Read, ATT_B_CV);
